...
parser.add_argument('--mu1', type=float, default=1.5)
parser.add_argument('--mu2', type=float, default=1.5)
parser.add_argument('--num_classes', type=int, default=40)
parser.add_argument('--drop_feature_rate_1', type=float, default=0.6)
...
parser.add_argument('--w1', type=float, default= 0.5, help='dynatic loss')
parser.add_argument('--w2', type=float, default= 0.5, help='dynatic loss')
parser.add_argument('--Alpha', type=float, default=0.6, help='LP wegiht')
...
parser.add_argument('--only_stage1', type=bool, default=False, help='whether to use eonly_stage1 in noise labels')
...
if __name__ == "__main__":
    ...
    for round_id in range(args.num_rounds):
        ...
        if global_acc_val > best_server_val:
            ...
            best_round = round_id
        global_acc_test_list.append(global_acc_test)
        ...
        if round_id<args.warm_up:
            for client_id in range(args.num_clients):
                ...
                for epoch_id in range(args.num_epochs):
                    ...
                    con_loss = 0.5 * (local_models[client_id].con_loss(x1, x2, args.temperature1) + local_models[client_id].con_loss(x2, x1,args.temperature1))
                    x_o = local_models[client_id].forward(subgraph)
                    supc_loss = loss_fn(x_o[train_idx], y[train_idx])
                    loss = (con_loss) * args.alpha1 + supc_loss
            ...
            if not warm_up_done or round_id % 10== 0:
